...
def get_best_actions(policy, states):
    policy.eval()
    with torch.no_grad():
        dist = policy(states)[0]
        best_actions = dist.sample()
    policy.train()
    return torch.clamp(best_actions,-1,1)
    

def collect_trajectories(env, brain_name, agent, num_agents):
    env_info = env.reset(train_mode=True)[brain_name]
    STATES = torch.empty(agent.buffer_size, num_agents, agent.state_size).to(device, torch.float)
    ACTIONS = torch.empty(agent.buffer_size, num_agents, agent.action_size).to(device, torch.float)
    REWARDS = torch.zeros(agent.buffer_size, num_agents).to(device, torch.float)
   
    for t in range(agent.buffer_size):
        STATES[t] = torch.from_numpy(env_info.vector_observations).to(device, torch.float)
        actions = get_best_actions(agent.policy, STATES[t])
        ACTIONS[t] = actions.to(torch.float)
    ...
    for k in range(num_agents):
        means = agent.policy(STATES[:,k,:])[1]
        stds = agent.policy.stds[0]
        LOG_PROBS = get_log_probs(ACTIONS[:,k,:], means, stds)
        VALUES = agent.critic(STATES[:,k,:])
        ADVANT, RETURNS = get_adv_and_returns(REWARDS[:,k], VALUES)
        score = torch.sum(REWARDS[:,k])
        results.append((STATES[:,k,:], ACTIONS[:,k,:], LOG_PROBS.detach(), RETURNS, ADVANT, VALUES.clone(), score))
